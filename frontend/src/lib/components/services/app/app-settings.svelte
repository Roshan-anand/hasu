<script lang="ts">
	import {
		...
		useUpdateServiceDomainMutation
	...
	const settingsQuery = useGetAppServiceDetailsQuery(() => serviceID);
	...
	let initialDomain = $state('');
	let initialPort = $state('');

	$effect(() => {
		if (settingsQuery.data) {
			isPublic = settingsQuery.data.is_public;
			initialIsPublic = settingsQuery.data.is_public;
			domainInput = settingsQuery.data.domain;
			initialDomain = settingsQuery.data.domain;
			portInput = settingsQuery.data.port.toString();
			initialPort = settingsQuery.data.port.toString();
		}
	});

	const isDirty = $derived(
		isPublic !== initialIsPublic || domainInput !== initialDomain || portInput !== initialPort
	);
